...
var fluidValuePattern = /^[\w.-]+\/[\w.-]+$/;
var arbitraryFluidValuePattern = /^\[[\w.]+\/[\w.]+\]$/;
function isFluidValue(value) {
  return fluidValuePattern.test(value);
}
function isArbitraryFluidValue(value) {
  return arbitraryFluidValuePattern.test(value);
}
function isAnyFluidValue(value) {
  return isFluidValue(value) || isArbitraryFluidValue(value);
}
function isNegativeFluidValue(value) {
  if (value.startsWith("-")) {
...
function createTwMerge(additionalConfig) {
  if (additionalConfig) {
...
function createFluidMergeConfig(options = {}) {
...
function createPrefixedTwMerge(options = {}) {
  const config = createFluidMergeConfig(options);
  return extendTailwindMerge(
    config
  );
}
function validateFluidClass(className) {
...
function filterValidFluidClasses(classString) {
  return classString.split(/\s+/).filter((cls) => {
    if (!cls.includes("fl-")) return true;
    return validateFluidClass(cls).valid;
  }).join(" ");
}
function twMergeWithValidation(...classLists) {
  const combined = classLists.filter(Boolean).join(" ");
  const filtered = filterValidFluidClasses(combined);
  return twMerge(filtered);
}

export { createFluidMergeConfig, createPrefixedTwMerge, createTwMerge, filterValidFluidClasses, isAnyFluidValue, isArbitraryFluidValue, isFluidValue, isNegativeFluidValue, twMerge, twMergeWithValidation, validateFluidClass, withFluid };
